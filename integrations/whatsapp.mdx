...
### Opening with a photo or a file

A template can open with an image, a PDF or a video above the text. Choose **Photo or file** when you add the template and pick one from your file library.

- **Image** — JPEG or PNG, up to 5 MB
- **Document** — PDF, up to 100 MB
- **Video** — MP4, up to 16 MB

The file is part of what Meta approves, so it is fixed once the template is approved: to send a different one, create another template. Every message that uses the template carries the file — including an [announcement](/campaign/announcements), which is how a photo reaches your customers *with* the offer rather than after they reply.

<Note>
    Changing the file later means a new template and a new Meta review. Pick something that stays true for as long as the template will be used — a logo or a product shot rather than this week's price.
</Note>

### Generating a Template with AI

Rather than writing a template and discovering at approval time that Meta rejects it, press **Generate with AI**.

Whappy writes a WhatsApp-compliant template from what you describe. **You review it before anything is sent to Meta** — nothing is submitted until you press Save.

**Configuration:**
- **What should the template say?** — a plain-language brief, for example *"Remind the customer about their appointment tomorrow and ask them to confirm"*. This is the only thing the AI writes from unless you add a campaign below.
- **Use a campaign's business context** *(optional)* — pick a campaign to add its business name, description and context documents to what the AI writes from. It starts empty, so no campaign is used unless you choose one. Any campaign can be picked, whether or not it currently runs on this number. With a campaign selected you may leave the brief empty and generate from its context alone.
- **Include a header** — a short title above the message, max 60 characters, no parameters.
- **Include a footer** — a small line below the message, max 60 characters, no parameters.
- **Include quick-reply buttons** — up to three tappable replies, max 25 characters each.

<Note>
    If the generated template needed changes to satisfy WhatsApp's rules, Whappy tells you how many adjustments were made. Read the result before saving — it is your template, and you are the one Meta holds to it.
</Note>
...
Need help? Check our [FAQ](/support/faq) or [contact support](/support/contact-support).